...
[![GitHub Actions](https://github.com/trinhminhtriet/spiko/workflows/CI/badge.svg)](https://github.com/trinhminhtriet/spiko/actions?query=workflow%3ACI)
[![Crates.io](https://img.shields.io/crates/v/spiko.svg)](https://crates.io/crates/spiko)
[![Arch Linux](https://img.shields.io/archlinux/v/extra/x86_64/spiko)](https://archlinux.org/packages/extra/x86_64/spiko/)
[![Homebrew](https://img.shields.io/homebrew/v/spiko)](https://formulae.brew.sh/formula/spiko)
[![Gitter](https://img.shields.io/gitter/room/trinhminhtriet/spiko)](https://gitter.im/trinhminhtriet-spiko/community#)

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/trinhminhtriet)

spiko is a tiny program that sends some load to a web application and show realtime tui inspired by [rakyll/hey](https://github.com/rakyll/hey).
...
You can optionally build spiko against [native-tls](https://github.com/sfackler/rust-native-tls) instead of [rustls](https://github.com/rustls/rustls).

    cargo install --no-default-features --features rustls spiko

You can enable VSOCK support by enabling `vsock` feature.

    cargo install --features vsock spiko
...
## On Windows (winget)
    ...
    winget install trinhminhtriet.spiko
...
You can also build and create a container image including spiko

```sh
docker build . -t example.com/trinhminhtriet/spiko:latest
```

Then you can use spiko directly through the container

```sh
docker run -it example.com/trinhminhtriet/spiko:latest https://example.com:3000
```

## Profile-Guided Optimization (PGO)

You can build `spiko` with PGO by using the following commands:

```sh
bun run pgo.js
```

And the binary will be available at `target/[target-triple]/pgo/spiko`.
...
`spiko` prints JSON output when `-j` option is set.
...
We used `hyperfine` for benchmarking `spiko` against `rakyll/hey` on a local server. The server was coded using node. You can start the server by copy pasting this file and then running it via node. After copy-pasting the file, you can run the benchmark via `hyperfine`.
...
2. Run `node app.js`
3. Run `hyperfine 'spiko --no-tui http://localhost:3000' 'hey http://localhost:3000'` in a different terminal tab

### Benchmark Results

Benchmark 1: spiko --no-tui http://localhost:3000
...
In this benchmark, `spiko --no-tui http://localhost:3000` was found to be faster, running approximately 1.32 ± 0.48 times faster than `hey http://localhost:3000`.

# Tips

## Stress test in more realistic condition

`spiko` uses default options inherited from [rakyll/hey](https://github.com/rakyll/hey) but you may need to change options to stress test in more realistic condition.

I suggest to run `spiko` with following options.

```sh
spiko <-z or -n> -c <number of concurrent connections> -q <query per seconds> --latency-correction --disable-keepalive <target-address>
```
...
```sh
spiko -n 10 --burst-delay 2s --burst-rate 4
```
...
_NOTE: If you don't set `--burst-rate` option, the amount is default to 1_
...
```sh
spiko --rand-regex-url http://127.0.0.1/[a-z][a-z][0-9]
```
